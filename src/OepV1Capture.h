...
// PARLIO RX. This implementation picks: one-shot and repeat, immediate trigger only, pushes events when subscribed.
// Repeat (as wch-protocols E078): PARLIO fills a 64 KiB internal DMA ring by partial receive; the ISR queues each
// finished chunk; a harvest task on core 0 copies it into K segments in PSRAM before the ring comes round again.
//
...
class LogicCapture final : public Interface {
 public:
  ...
  static constexpr size_t kRingBytes = 64 * 1024;                     // repeat: the DMA ring (internal)
  static constexpr size_t kStoreMax = 16 * 1024 * 1024;               // repeat: segments in PSRAM
  static constexpr size_t kSegmentMin = 4096, kSegmentMaxRepeat = 1024 * 1024;
  static constexpr size_t kInfos = 64;                                // segment infos kept
