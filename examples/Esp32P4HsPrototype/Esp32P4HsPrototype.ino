...
static const oep::v1::ProbeConfig::Mode kModes[] = {
    {0x07, 1, "debug"},   // vendor bulk, CDC OEP port, HID OEP port; one data port
    {0x01, 0, "logic"},   // vendor bulk only
};
static const uint8_t kPortInterfaces[] = {4};   // mode 0: HID 0, vendor 1, CDC OEP 2-3, data port 0 = 4-5
static oep::v1::ProbeConfig config(endpoint, kModes, 2, kPortInterfaces);
...
void setup() {
  ...
  if (mode == 0) {   // functions are created before the device starts; interface numbers follow this order
    hid = new EspUsbDeviceHidVendor(usbDevice, 511);
    cdc = new EspUsbDeviceCdcSerial(usbDevice, "OEP control (CDC)");
    bridgePort = new EspUsbDeviceCdcSerial(usbDevice, "UART bridge");
    hidStream = new HidStream(*hid);
    cdcStream = new CdcStream(*cdc);
    portStream = new PortStream(*bridgePort);
  ...
  if (mode == 0) {
    endpoint.addTransport(*hidStream, rxC, sizeof rxC, oep::v1::Endpoint::Framing::kLengthPrefixed, true);
    endpoint.addTransport(*cdcStream, rxB, sizeof rxB, oep::v1::Endpoint::Framing::kLengthPrefixed, true);
