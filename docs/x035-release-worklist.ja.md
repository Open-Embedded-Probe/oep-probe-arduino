...
| --- | --- | --- | --- |
| 1 | GPIO / `INPUT_PULLUP` / EXTI | `fixture.gpio`（両側駆動・観測） | 全接続 pin の 0/1、pull、edge、割込み回数、予約 pin 非干渉。**2026-09-22 実測**（ArduinoCore-CH32 `tests/manual/oep_gpio_matrix/`、13 pin）: core の 2 不具合を発見・修正 — (a) AFIO_EXTICR を F1 流 4 bit で書いていて port B/C の EXTI が来ない（X035 は 2 bit/line）、(b) X0 の GPIO に汎用 open-drain が無く `OUTPUT_OPENDRAIN` が high を駆動（errata `x035-no-gpio-open-drain`、core でエミュレート）。修正後 PA0〜7 / PB3 / PB11 / PB12 は全項目 OK。PC14/PC15（PD CC）は pull-up idle が 0 のまま（≈5 kΩ 級 pull-down、源は未決）。**同日（続き）** PC16/PC17 を追加: out / od / in / pull は OK、EXTI は core が line 16〜23 を切っていて 0 → generator に `CH32_EXTI_LINES` 24 と `EXTI25_16` group を出させて 10/10/20 |
...
| 3 | ADC | 校正済み 0/Vref/2/Vref source | PA5 を含む接続 ADC pin の許容範囲、settling、repeatability。P4 内部 pull は基準電圧にしない。**2026-09-22 実測**（ArduinoCore-CH32 `tests/manual/oep_adc_trace/`、rail 端点のみ）: PA0/1/2/4/5/6 は low 1 / high 1008（16 回のばらつき ≤ 4）。**PA3 / PA7 / VREFINT（ch3/7/15）はこの F8U6 に無い**（errata `x035-adc-ch-i2c-unavailable` の ADC 条項。I2C 条項は当たらず I2C は動く）。無い channel は直前の変換の残留を読んで減衰するので、判定は VREFINT を PA0 両 rail で前置して行う。中点は P4 の両 pull（≈1.47 V、E087）で 451〜463 / 1023（期待 ≈455）、概略として取得。直線性は未 |
...
| 6 | I2C master / `Wire` route 2 | `p4.i2c-target` + `fixture.capture`（同一 plan） | 10/100 kHz write/read、repeated START、NACK、clock stretch、stuck-bus recovery。**2026-09-22 実測**（ArduinoCore-CH32 `tests/manual/oep_i2c_trace/`）: write 10/100/400 kHz ACK・受信一致、NACK（無応答 address）、read 4 byte × 2 slot 一致、repeated START 一致。core の 2 バグを修正 — `USB_PHY_V33` で open-drain が release されない（errata `x035-usb-pads-open-drain`）、Wire の `NoIrq` が User mode で `mstatus` を触って `requestFrom` が trap で永久停止。**同日（続き）**: `p4.i2c-target.set_stretch` で target に SCL を伸ばさせ、0.1〜20 ms は Wire が待って一致、30 ms は 25 ms（`CH32_WIRE_TIMEOUT_US`）で rc/got=0 と timeout flag、次で回復。stuck bus: P4 が SDA/SCL を固定 → rc=5（SDA は 36 µs の error flag 経路、SCL は 25 ms timeout）、解放で回復。target が byte 途中で SDA を握る → Wire は 25 ms timeout を繰り返し **SWRST では解放できない**、sketch 側の 9 pulse bus clear で解放。core に bus clear API を置くかは未決（todo）。fixture の route 2 に bus pull-up が無いことも判明 |
| 7 | SPI master | `fixture.capture`（peer slave は未） | CPOL/CPHA、CS、MOSI/MISO、clock、連続 transfer、route、bus release。**2026-09-22 実測**: mode 0〜3 とも CPOL と MOSI 遷移エッジが仕様どおり、data 一致、prescaler 64/16/256。MISO 応答（peer slave）は未。**同日（続き）**: P4 に `p4.spi-target`（SPI2_HOST slave、64 byte）を足し、mode 0〜3 @0.74 MHz、mode 0 @0.19 MHz、mode 0/3 @2.86 MHz で DUT の MISO 受信・target の MOSI 受信・線上 decode の 3 者が一致（ArduinoCore-CH32 `oep_periph_trace --only spi-peer`）。12 MHz 要求（実 6 MHz）と 24 MHz 要求（実 24 MHz、/2）でも data は両方向一致（capture は 4 MHz まで）。**同日**: 64 byte transaction × 3 と 4 byte × 5 連続（4 MHz mode 3）も両方向一致 |
| 8 | GPIO alternate/remap | route_selftest（API）+ 各 trace runner（線上） | 各 API の route 指定が予約/他 peripheral を壊さず、失敗を明示すること。**2026-09-22**: API 側は basic `route_selftest` 7/7。線上では I2C route 2 / 4 と USART2 を使う runner が全て console（USART4）と SWD を保ったまま通っている。無効 route の明示失敗（`setRoute` false）は `route_selftest` で確認。**同日**: I2C route 4（PC17/PC16、SCL/SDA 交換）も線上 decode 付きで一致。配線のある route（I2C 2/4、USART2/4、SPI 既定）は全部通した。配線の無い route は fixture では試験不能 |
| 9 | reset/startup/boot interaction | `fixture.capture` + GPIO marker | normal reset、software reset、boot entry の範囲、probe disconnect 時の安全状態。**2026-09-22 実測**（ArduinoCore-CH32 `tests/manual/oep_reset_trace/`）: `CH32.restart()` → setup() **0.184 ms**、probe の debug reset → setup() 7〜139 ms（probe 列が支配）、debug reset 後の `resetReason` は software（区別不能）。probe disconnect は watchdog → target reset で reliability gate 5/5。watchdog / software reset の reason は basic `system_selftest` で 11/11。NRST pin reset は配線が無く未 |
