...
| 3 | ADC | 校正済み 0/Vref/2/Vref source | PA5 を含む接続 ADC pin の許容範囲、settling、repeatability。P4 内部 pull は基準電圧にしない。**2026-09-22 実測**（ArduinoCore-CH32 `tests/manual/oep_adc_trace/`、rail 端点のみ）: PA0/1/2/4/5/6 は low 1 / high 1008（16 回のばらつき ≤ 4）。**PA3 / PA7 / VREFINT（ch3/7/15）はこの F8U6 に無い**（errata `x035-adc-ch-i2c-unavailable` の ADC 条項。I2C 条項は当たらず I2C は動く）。無い channel は直前の変換の残留を読んで減衰するので、判定は VREFINT を PA0 両 rail で前置して行う。中点・直線性は校正 source が無く未 |
